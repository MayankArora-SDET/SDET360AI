...
function mainScript() {
  ...
  var inputValues = {};
  var eventsToSend =
    sessionStorage.getItem("eventsToSend") &&
    sessionStorage.getItem("eventsToSend") !== ""
  ...
  if (sessionStorage.getItem("action") == null) {
    //this data storage is it send in api to create new array or to append
  ...
  setTimeout(() => {
    console.log(
      sessionStorage.getItem("eventsToSend"),
      sessionStorage.getItem("isRecording")
    );
  }, 2000);
  ...
  function createElement() {
    ...
    setTimeout(() => {
      ...
      toggleRecordingButton.textContent =
        sessionStorage.getItem("isRecording") == "true"
          ? "Pause Recording"
          : "Resume Recording";
  ...
  function dragElement(elmnt) {
    ...
    function elementDrag(e) {
      ...
      elmnt.style.right = elmnt.offsetRight - pos3 + "px";
      elmnt.style.bottom = elmnt.offsetBottom - pos4 + "px";
    ...
    try {
      ...
      try {
        const response = await fetch(
          "http://tenant1.localhost:8080/api/tokens/record",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              // Add headers that might help bypass ad blockers
        ...
        console.warn("Direct fetch failed, trying via background script:", directFetchError);
        
        // Second try: Send via background script to avoid content blockers
        return new Promise((resolve, reject) => {
          chrome.runtime.sendMessage(
            {
              action: "sendApiRequest",
              url: "http://tenant1.localhost:8080/api/tokens/record",
              method: "POST",
              data: payload,
            },
            (response) => {
              if (chrome.runtime.lastError) {
                ...
                console.log("API request successful via background script");
      ...
      console.error("All attempts to send data to API failed:", e);
      throw e;
    }
  }
  ...
  function getRelativeXpath(element) {
    ...
    function getNodeXPath(node) {
      ...
      if (attributes.length > 0)
        return (
          node.nodeName.toLowerCase() + "[" + attributes.join(" and ") + "]"
        );
